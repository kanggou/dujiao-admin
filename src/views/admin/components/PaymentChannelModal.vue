...
const payssionConfig = reactive({
  api_key: '',
  secret_key: '',
  gateway_url: 'https://www.payssion.com/api/v1/payment',
  notify_url: '',
  return_url: '',
  pm_id: '',
  target_currency: '',
...
const payssionChannelOptions = [
  { value: 'payssion', label: 'admin.paymentChannels.channelTypes.payssion' },
]
...
const formChannelOptions = computed(() => {
  ...
  if (form.provider_type === 'payssion') {
    return payssionChannelOptions
  }
...
const resetPayssionConfig = () => {
  payssionConfig.api_key = ''
  payssionConfig.secret_key = ''
  payssionConfig.gateway_url = 'https://www.payssion.com/api/v1/payment'
  payssionConfig.notify_url = 'https://api.yourdomain.com/api/v1/payments/callback'
  payssionConfig.return_url = 'https://yourdomain.com/pay'
  payssionConfig.pm_id = ''
  payssionConfig.target_currency = ''
  payssionConfig.exchange_rate = ''
}
...
const applyPayssionConfig = (raw: Record<string, unknown>) => {
  ...
  payssionConfig.pm_id = String(raw.pm_id || '')
  payssionConfig.target_currency = String(raw.target_currency || '')
  payssionConfig.exchange_rate = String(raw.exchange_rate || '')
}
...
const buildPayssionConfig = () => {
  ...
  setIfNotEmpty('api_key', payssionConfig.api_key)
  setIfNotEmpty('secret_key', payssionConfig.secret_key)
  setIfNotEmpty('gateway_url', payssionConfig.gateway_url)
  setIfNotEmpty('notify_url', payssionConfig.notify_url)
  setIfNotEmpty('return_url', payssionConfig.return_url)
  setIfNotEmpty('pm_id', payssionConfig.pm_id)
  setIfNotEmpty('target_currency', payssionConfig.target_currency)
  setIfNotEmpty('exchange_rate', payssionConfig.exchange_rate)
  return config
}
...
watch(
  ...
  (value) => {
    ...
    } else if (value === 'payssion') {
      form.channel_type = 'payssion'
    ...
    if (id === null) {
      ...
      try {
        ...
        if (channel.config_json && typeof channel.config_json === 'object') {
          ...
          applyPayssionConfig(channel.config_json)
        } else {
          resetAllConfigs()
        }
      } catch (err: any) {
...
const handleSubmit = async () => {
  ...
  } else if (form.provider_type === 'payssion') {
    delete configJson.target_currency
    delete configJson.exchange_rate
    configJson = {
      ...configJson,
          ...
          <div class="min-w-0">
            ...
            <Select v-model="form.provider_type">
              ...
              <SelectContent>
                ...
                <SelectItem value="payssion">{{ t('admin.paymentChannels.providerTypes.payssion') }}</SelectItem>
              </SelectContent>
            </Select>
          </div>
        ...
        <div v-if="form.provider_type === 'payssion'" class="min-w-0 rounded-xl border border-border bg-muted/20 p-4 overflow-hidden">
          ...
          <div class="grid grid-cols-1 gap-4 md:grid-cols-2 [&>*]:min-w-0">
            <div class="min-w-0">
              ...
              <Input v-model="payssionConfig.api_key" placeholder="Your Payssion API Key" />
            </div>
            <div class="min-w-0">
              <label class="block text-xs font-medium text-muted-foreground mb-1.5">Secret Key</label>
              <Input v-model="payssionConfig.secret_key" placeholder="Your Payssion Secret Key" />
            </div>
            <div class="min-w-0 md:col-span-2">
              <label class="block text-xs font-medium text-muted-foreground mb-1.5">Gateway URL</label>
              <Input v-model="payssionConfig.gateway_url" placeholder="https://www.payssion.com/api/v1/payment" />
            </div>
            <div class="min-w-0">
              <label class="block text-xs font-medium text-muted-foreground mb-1.5">PM ID</label>
              <Input v-model="payssionConfig.pm_id" placeholder="sofort, ideal, boleto..." />
            </div>
            <div class="min-w-0">
              <label class="block text-xs font-medium text-muted-foreground mb-1.5">Notify URL</label>
              <Input v-model="payssionConfig.notify_url" placeholder="https://api.yourdomain.com/api/v1/payments/callback" />
            </div>
            <div class="min-w-0">
              <label class="block text-xs font-medium text-muted-foreground mb-1.5">Return URL</label>
              <Input v-model="payssionConfig.return_url" placeholder="https://yourdomain.com/order/success" />
            </div>
            <div class="min-w-0">
              <label class="block text-xs font-medium text-muted-foreground mb-1.5">{{ t('admin.paymentChannels.modal.targetCurrency') }}</label>
              <Input v-model="payssionConfig.target_currency" :placeholder="t('admin.paymentChannels.modal.targetCurrencyPlaceholder')" />
            </div>
            <div class="min-w-0">
              <label class="block text-xs font-medium text-muted-foreground mb-1.5">{{ t('admin.paymentChannels.modal.exchangeRate') }}</label>
              <Input v-model="payssionConfig.exchange_rate" :placeholder="t('admin.paymentChannels.modal.exchangeRatePlaceholder')" />
            </div>
          </div>
          <div class="mt-3 text-xs text-muted-foreground">PM ID 为 Payssion 支持的支付方式标识，如 sofort、ideal、boleto 等。留空时将使用渠道类型作为 pm_id。</div>
        </div>
