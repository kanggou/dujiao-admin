...
const props = withDefaults(defineProps<{
  channelType: 'telegram_bot' | 'discord_bot'
  i18nPrefix: 'telegramBot' | 'discordBot'
  defaultName?: string
}>(), {
  defaultName: '',
})
...
const tt = (key: string) => t(`${props.i18nPrefix}.channelClients.${key}`)
...
const fetchClients = async () => {
  ...
  try {
    ...
    clients.value = Array.isArray(data)
      ? data.filter((client: ChannelClient) => client.channel_type === props.channelType)
      : []
  } catch {
    clients.value = []
  } finally {
...
const handleCreate = async () => {
  creating.value = true
  try {
    await adminAPI.createChannelClient({
      ...createForm.value,
      channel_type: props.channelType,
    })
    notifySuccess(tt('createSuccess'))
    showCreateDialog.value = false
    createForm.value = createDefaultForm()
    fetchClients()
  } catch {
    notifyError(tt('createFailed'))
  } finally {
...
const handleEdit = async () => {
  ...
  try {
    ...
    notifySuccess(tt('editSuccess'))
    showEditDialog.value = false
    fetchClients()
  } catch {
    notifyError(tt('editFailed'))
  } finally {
    editing.value = false
  }
}
...
const handleToggleStatus = async (client: ChannelClient) => {
  ...
  try {
    ...
    notifySuccess(tt('statusUpdated'))
    fetchClients()
  } catch {
    notifyError(tt('statusUpdateFailed'))
  }
}
...
const handleConfirmAction = async () => {
  ...
  try {
    if (confirmAction.value === 'reset') {
      ...
      notifySuccess(tt('resetSecretSuccess'))
    } else if (confirmAction.value === 'delete') {
      await adminAPI.deleteChannelClient(confirmTargetClient.value.id)
      notifySuccess(tt('deleteSuccess'))
    }
    closeConfirm()
    fetchClients()
  } catch {
    notifyError(confirmAction.value === 'reset' ? tt('resetSecretFailed') : tt('deleteFailed'))
  } finally {
...
<template>
  <div class="space-y-6">
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div>
        <h2 class="text-2xl font-bold tracking-tight">{{ tt('title') }}</h2>
        <p class="text-muted-foreground">{{ tt('subtitle') }}</p>
      </div>
        ...
        <Table class="min-w-[920px]">
          <TableHeader>
            <TableRow>
              ...
              <TableHead class="min-w-[160px]">{{ tt('name') }}</TableHead>
              <TableHead class="min-w-[160px]">{{ tt('channelKey') }}</TableHead>
              <TableHead class="min-w-[160px]">{{ tt('channelSecret') }}</TableHead>
              <TableHead class="min-w-[100px]">{{ tt('botToken') }}</TableHead>
              <TableHead class="min-w-[180px]">{{ tt('callbackUrl') }}</TableHead>
              <TableHead class="min-w-[90px]">{{ tt('statusLabel') }}</TableHead>
              <TableHead class="min-w-[160px]">{{ tt('actions') }}</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            ...
            <TableRow v-for="client in clients" :key="client.id">
              ...
              <TableCell class="min-w-[100px]">
                ...
                <span v-else class="text-xs text-muted-foreground">{{ tt('botTokenNotSet') }}</span>
              </TableCell>
              <TableCell class="min-w-[180px]">
                ...
                <span v-else class="text-xs text-muted-foreground">{{ tt('callbackUrlNotSet') }}</span>
              </TableCell>
              <TableCell class="min-w-[90px]">
                <Badge :variant="client.status === 1 ? 'default' : 'secondary'">
                  {{ client.status === 1 ? tt('active') : tt('disabled') }}
                </Badge>
              </TableCell>
              <TableCell class="min-w-[160px]">
                <div class="flex flex-wrap items-center gap-1">
                  <Button variant="outline" size="sm" @click="openEditDialog(client)" :title="tt('edit')">
                    <Pencil class="h-3.5 w-3.5" />
                  </Button>
                  <Button variant="outline" size="sm" @click="handleToggleStatus(client)">
                    {{ client.status === 1 ? tt('disable') : tt('enable') }}
                  </Button>
                  <Button variant="outline" size="sm" @click="openConfirm('reset', client)" :title="tt('resetSecret')">
                    <RotateCcw class="h-3.5 w-3.5" />
                  </Button>
                  <Button variant="outline" size="sm" class="text-destructive hover:text-destructive" @click="openConfirm('delete', client)" :title="tt('delete')">
    ...
    <Dialog v-model:open="showCreateDialog">
      <DialogContent class="w-[calc(100vw-1rem)] max-w-lg p-4 sm:p-6">
        <DialogHeader>
          <DialogTitle>{{ tt('createTitle') }}</DialogTitle>
          <DialogDescription>{{ tt('createDesc') }}</DialogDescription>
        </DialogHeader>
        <div class="space-y-4">
          <div class="space-y-2">
            <Label>{{ tt('name') }}</Label>
            <Input v-model="createForm.name" :placeholder="tt('namePlaceholder')" />
          </div>
          <div class="space-y-2">
            <Label>{{ tt('botToken') }}</Label>
            <Input v-model="createForm.bot_token" :placeholder="tt('botTokenPlaceholder')" />
            <p class="text-xs text-muted-foreground">{{ tt('botTokenHint') }}</p>
          </div>
          <div class="space-y-2">
            <Label>{{ tt('callbackUrl') }}</Label>
            <Input v-model="createForm.callback_url" :placeholder="tt('callbackUrlPlaceholder')" />
            <p class="text-xs text-muted-foreground">{{ tt('callbackUrlHint') }}</p>
          </div>
          <div class="space-y-2">
            <Label>{{ tt('description') }}</Label>
            <Textarea v-model="createForm.description" :placeholder="tt('descriptionPlaceholder')" rows="2" />
          </div>
        </div>
        <DialogFooter class="flex-col-reverse sm:flex-row">
          <Button class="w-full sm:w-auto" variant="outline" @click="showCreateDialog = false">{{ tt('cancel') }}</Button>
    ...
    <Dialog v-model:open="showEditDialog">
      <DialogContent class="w-[calc(100vw-1rem)] max-w-lg p-4 sm:p-6">
        ...
        <div class="space-y-4">
          <div class="space-y-2">
            <Label>{{ tt('name') }}</Label>
            <Input v-model="editForm.name" :placeholder="tt('namePlaceholder')" />
          </div>
          <div class="space-y-2">
            <Label>{{ tt('botToken') }}</Label>
            <Input v-model="editForm.bot_token" :placeholder="tt('botTokenEditPlaceholder')" />
            <p class="text-xs text-muted-foreground">
              <template v-if="editingClient?.bot_token_set">
                {{ tt('botTokenCurrentSet') }}
              </template>
              <template v-else>
                {{ tt('botTokenCurrentNotSet') }}
              </template>
            </p>
          </div>
          <div class="space-y-2">
            <Label>{{ tt('callbackUrl') }}</Label>
            <Input v-model="editForm.callback_url" :placeholder="tt('callbackUrlPlaceholder')" />
            <p class="text-xs text-muted-foreground">{{ tt('callbackUrlHint') }}</p>
          </div>
          <div class="space-y-2">
            <Label>{{ tt('description') }}</Label>
            <Textarea v-model="editForm.description" :placeholder="tt('descriptionPlaceholder')" rows="2" />
          </div>
        </div>
        <DialogFooter class="flex-col-reverse sm:flex-row">
          <Button class="w-full sm:w-auto" variant="outline" @click="showEditDialog = false">{{ tt('cancel') }}</Button>
    ...
    <Dialog :open="!!confirmAction" @update:open="(v: boolean) => { if (!v) closeConfirm() }">
      <DialogContent class="w-[calc(100vw-1rem)] max-w-md p-4 sm:p-6">
        <DialogHeader>
          <DialogTitle>
            {{ confirmAction === 'reset' ? tt('resetSecretTitle') : tt('deleteTitle') }}
          </DialogTitle>
          <DialogDescription>
            {{ confirmAction === 'reset' ? tt('resetSecretDesc') : tt('deleteDesc') }}
          </DialogDescription>
        </DialogHeader>
        <DialogFooter class="flex-col-reverse sm:flex-row">
          <Button class="w-full sm:w-auto" variant="outline" @click="closeConfirm">{{ tt('cancel') }}</Button>
